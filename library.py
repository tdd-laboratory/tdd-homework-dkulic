...
_date_string_pat = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    '''Find iso8601 dates, eg 2015-07-25'''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_string_month(text):
    '''Find text dates, eg 25 Jan 2017'''
    for match in _date_string_pat.finditer(text):
        yield('date', match)
